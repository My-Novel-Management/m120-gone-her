...
def sc_searching(w: World):
    ...
    return w.scene("捜索",
            ...
            yuki.do("$full_arisuがいた頃なら廊下から中を覗く同級生や下級生が固まっている姿も珍しくないくらい、",
                "その教室の中と外には彼女たちの体温があったのに、エアコンすら点いていない", "&"),
            ...
            yuki.hear("廊下を誰かが駆けてくる音が、佇んでいた$Sの意識に響いた"),
...
def sc_underground(w: World):
    ...
    return w.scene("裏掲示板の存在",
            ...
            inside.look("重い木戸を開け、喋り声の一つも響かないその部屋に入る",
                "窓も小さくおまけに薄いカーテンが引かれたままで、蛍光灯は半分しか点けられていない",
            ...
            yuki.do("本名は知らない", "誰が名付けたのかその風貌から$franという名で呼ばれているこの学校唯一の司書は、",
                ...
                "いつ見ても本の手入ればかりしていて、まるでそれだけが彼の業務内容のように$Sには思えた"),
            _.do("それでも$franの存在が多くの生徒を遠ざけてくれるお陰で、",
            ...
            yuki.think("$S自身の紹介文はもちろん自分で書いたものだ",
                "そういえば一度何かの用事で$Sたちのクラスにやってきた$full_arisuが、何故か$Sのそれの前で立ち止まり眺めていたことがあったが、",
                "あの時は流石に自分の心臓がおかしくなるんじゃないかと思ったことを、今思い出した"),
            yuki.do("図書館の一番奥、そこに貸出禁止の書架がある",
                "学校の歴史や分厚い資料",
                "今までの学生の記録など",
            ...
            yuki.think("裏掲示板――という言葉が浮かび上がったが、"),
            fran.talk("何だ？"),
            yuki.do("$franの声と足音がしたので$CSは慌てる",
                    "とりあえず棚に戻そうと思ったが、その前に目に留まった一文のあるページを破り取り、",
                "栞と一緒にスカートのポケットに仕舞い込んだ"),
            fran.talk("何をしている？"),
            yuki.talk("手が当たって落としてしまって。すみません"),
            fran.talk("大事なものばかりだから丁寧に扱って下さい"),
            fran.do("不気味だ、という先入観を取り払った彼は小さく頭を下げると、何もせずにカウンターの方へと戻って行ってしまった"),
            yuki.do("完全に見えなくなってから$Sは手にした本を棚へと戻し、入り口に向かう",
                    "もう一度中身を見たい衝動に駆られたが、それよりもポケットの中のものを持ち去ることを$Sは選んだ"),
            _.do("もう一度中身を見たい衝動に駆られたが、今自分のポケットに入れたものを持ち去る方がずっと重要だと分かっていたので、",
                    "逃げるように早足で、再び羽根ばたきを手にしてカウンターに就いていた司書の前を素通りし、図書室を後にした").omit(),
            ## NOTE
